...
namespace VirtualMemoryProvider
{
    public sealed class FileSystemApi
    {
        ...
        {
            var rootParts = root.Split(Path.DirectorySeparatorChar);
            var pathParts = path.Split(Path.DirectorySeparatorChar);

            int i = 0;
            return string.Join(Path.DirectorySeparatorChar, pathParts.SkipWhile(x =>
            {
                bool res = rootParts.ElementAtOrDefault(i) == x;
